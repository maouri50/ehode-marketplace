...
export default function Account() {
  ...
  const requestPasswordReset = trpc.buyer.requestPasswordReset.useMutation({ onSuccess: (result) => setNotice(result.message) });
  ...
  const submitReview = trpc.storefront.reviews.submit.useMutation({ onSuccess: async (_, variables) => { setNotice("Thank you. Your verified review is now live."); setReviewDrafts((drafts) => { const copy = { ...drafts }; delete copy[variables.orderItemId]; return copy; }); await eligibleReviews.refetch(); } });
  ...
  async function submitResetRequest(event: FormEvent) {
    event.preventDefault(); setNotice("");
    try { await requestPasswordReset.mutateAsync({ email }); } catch { setNotice("If an account matches this email, a reset link is on its way."); }
  }
  ...
  if (!me.data) return <GuestAccount mode={mode} setMode={setMode} displayName={displayName} setDisplayName={setDisplayName} email={email} setEmail={setEmail} password={password} setPassword={setPassword} notice={notice} submitAuth={submitAuth} pending={register.isPending || login.isPending} forgotPassword={forgotPassword} setForgotPassword={setForgotPassword} submitResetRequest={submitResetRequest} resetPending={requestPasswordReset.isPending}/>;

  return <div className="store-page buyer-page"><StoreHeader/><main className="buyer-shell"><header className="buyer-dashboard-head"><div><p className="buyer-kicker">Buyer account</p><h1>Hello, {me.data.displayName}.</h1><p>Your saved resources, purchases, and genuine review invitations live here.</p></div><button className="buyer-quiet-button" type="button" onClick={() => logout.mutate()}><LogOut size={16}/> Log out</button></header>
