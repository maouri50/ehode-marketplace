...
describe("newsletter subscription helpers", () => {
  ...
  it("creates a new subscription once and treats an active duplicate as a safe success", async () => {
    const created: string[] = [];
    const activeStore = { findByEmail: async () => null, create: async (email: string) => { created.push(email); }, reactivate: async () => {} };
    await expect(subscribeNewsletter(activeStore, "  HELLO@EXAMPLE.COM ")).resolves.toEqual({ success: true, message: NEWSLETTER_SUCCESS_MESSAGE });
    ...
    await expect(subscribeNewsletter(duplicateStore, "hello@example.com")).resolves.toEqual({ success: true, message: NEWSLETTER_SUCCESS_MESSAGE });
  });

  it("reactivates a previous subscriber after fresh consent", async () => {
    ...
    await subscribeNewsletter(store, "returning@example.com");
