...
export function buildNewsletterCampaignEmail(input: { body: string; unsubscribeUrl: string; products?: NewsletterCampaignProduct[]; canonicalOrigin?: string; templateType?: NewsletterCampaignTemplate; seasonLabel?: string | null }) {
  ...
  const productHeading = input.templateType === "seasonal" && input.seasonLabel ? `Fresh ideas for ${escapeHtml(input.seasonLabel)}` : input.templateType === "latest" ? "Fresh ideas for your next project" : "A thoughtful edit for you";
  const productIntro = input.templateType === "seasonal" && input.seasonLabel ? `A small collection chosen for ${escapeHtml(input.seasonLabel)}.` : input.templateType === "latest" ? "A new collection of practical creative tools, ready when inspiration strikes." : "A few real Ehode resources selected to make your next project easier.";
  const productCards = products.length ? `<section style="margin:30px 0 0;padding:24px 20px;border-radius:16px;background:#fff8f1"><p style="margin:0 0 7px;color:#c95420;font-size:11px;font-weight:800;letter-spacing:1.7px;text-transform:uppercase">Ehode curated picks</p><h2 style="margin:0 0 8px;font:500 27px Georgia,serif;letter-spacing:-.35px;color:#2f2924">${productHeading}</h2><p style="margin:0 0 20px;color:#6c6259;font-size:14px;line-height:1.55">${productIntro}</p>${products.map((product) => {
    const link = absoluteUrl(origin, `/products/${encodeURIComponent(product.handle)}`);
    const cover = product.coverImageUrl ? `<a href="${escapeHtml(link)}" style="display:block;text-decoration:none"><img src="${escapeHtml(absoluteUrl(origin, product.coverImageUrl))}" alt="${escapeHtml(product.title)}" width="142" style="display:block;width:142px;max-width:100%;height:auto;border:0;border-radius:9px;background:#f2ece5"/></a>` : "";
    return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 12px;border:1px solid #eadfd5;border-radius:10px;background:#fffdf9"><tr><td style="padding:12px;vertical-align:top;width:154px">${cover}</td><td style="padding:14px 14px 14px 0;vertical-align:middle"><p style="margin:0 0 7px;font-size:16px;font-weight:700;line-height:1.35;color:#2f2924">${escapeHtml(product.title)}</p><p style="margin:0 0 13px;font-size:13px;color:#6c6259">${escapeHtml(product.currencyCode)} ${escapeHtml(product.priceAmount)}</p><a href="${escapeHtml(link)}" style="display:inline-block;color:#9d451d;font-size:13px;font-weight:800;text-decoration:none">See this resource →</a></td></tr></table>`;
  }).join("")}<p style="margin:20px 0 0;text-align:center"><a href="${escapeHtml(absoluteUrl(origin, "/#collection"))}" style="display:inline-block;padding:13px 21px;border-radius:8px;background:#d85e26;color:#fff;text-decoration:none;font-size:14px;font-weight:800">Explore the collection →</a></p></section>` : "";
  ...
  return {
    html: `<main style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;padding:0 12px 28px;color:#2f2a25;line-height:1.6;background:#f7f3ee"><div style="margin:0 -12px 24px;padding:12px 20px;background:#d85e26;color:#fff;text-align:center;font-size:11px;font-weight:800;letter-spacing:1.35px;text-transform:uppercase">Original digital goods for creative projects</div><section style="padding:24px 20px;background:#fffdf9;border-radius:0 0 16px 16px"><p style="margin:0 0 26px;font:700 23px Georgia,serif;letter-spacing:-.4px;color:#2f2924">ehode<span style="color:#d45b22">.</span></p><div style="font-size:16px;color:#443a33;line-height:1.65">${paragraphs}</div>${productCards}${footer}</section></main>`,
